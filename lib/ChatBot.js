...
import { Dimensions, Keyboard, TextInput, ScrollView, Platform,View,TouchableOpacity,Text } from 'react-native';
...
class ChatBot extends Component {
  ...
  onButtonPress(e,fromImagePicker=false) {
    const {
      renderedSteps,
      previousSteps,
      inputValue,
    ...
    let isInvalid = false;
    if(fromImagePicker){
      isInvalid=!imageValue
    }else{
      isInvalid=this.checkInvalidInput();
    }
  
console.log("isInvalid",isInvalid,currentStep.validator,fromImagePicker)
    if (!isInvalid) {
      const step = {
        message: inputValue,
        value: inputValue,
  ...
  async triggerNextStep(data) {
    ...
    if (isEnd) {
      ...
      const options = currentStep.options.filter(o => _.includes(data.value, o.value) );
      ...
      if (nextStep.user) {
        this.setState({showImagePicker:_.get(nextStep,"metadata.showImagePicker", false)});
  ...
  checkInvalidInput() {
    ...
    const result = currentStep.validator?currentStep.validator(inputValue):inputValue;
  ...
  renderStep(step, index) {
    ...
    for (let i = 0, len = previousSteps.length; i < len; i += 1) {
      ...
      steps[ps.id] = {
        ...
        message: ps.message,
        value: ps.value,
        selectionCount:ps.selectionCount
      };
    }
    ...
    if (options) {
      return (
        <OptionsStep
          ...
          selectedOptionElementStyle={selectedOptionElementStyle}
        />
      );
    }
    ...
    return (
      <TextStep
        ...
        renderCustomAvtarComponent={renderCustomAvtarComponent}
      />
    );
  }
...
onImagePickerClick = ()=>{
  ...
  render() {
    ...
    const {
      ...
      showInput,
      buttonFontColor,
      buttonDisableFontColor,
      imagePickerStyle,
      imagePickerContainerStyle,
      imagePickerTextStyle,
      imagePickerContainerDisableStyle,
      imagePickerDisableStyle,
      imagePickerTextDisableStyle,
      alwaysShowImagePicker
    } = this.props;
const showPicker =alwaysShowImagePicker||showImagePicker;
    const styles = {
      input: {
        borderWidth: 0,
        ...
        paddingLeft: 16,
        height: 50,
        width: width - (80 +(showPicker?40:0)),
    ...
    const imagePickerContainerCustomStyle = Object.assign({}, styles.imagePickerContainerStyle, imagePickerContainerStyle);
    const imagePickerCustomStyle = Object.assign({}, styles.imagePickerStyle, imagePickerStyle);
    const imagePickerTextCustomStyle = Object.assign({}, styles.imagePickerTextStyle, imagePickerTextStyle);
    const imagePickerDisableContainerCustomStyle = Object.assign({}, styles.imagePickerContainerDisableStyle, imagePickerContainerDisableStyle);
    const imagePickerDisableCustomStyle = Object.assign({}, styles.imagePickerDisableStyle, imagePickerDisableStyle);
    const imagePickerTextDisableCustomStyle = Object.assign({}, styles.imagePickerTextDisableStyle, imagePickerTextDisableStyle);
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {showPicker &&<View style={{...imagePickerContainerCustomStyle,...!editable?imagePickerDisableContainerCustomStyle:{}}}>
              <TouchableOpacity style={{...imagePickerCustomStyle,...!editable?imagePickerDisableCustomStyle:{}}} activeOpacity={0.8} onPress={this.onImagePickerClick}>
                <Text style={{...imagePickerTextCustomStyle,...!editable?imagePickerTextDisableCustomStyle:{}}}>{`+`}</Text>
                </TouchableOpacity>
            </View>}
            ...
            >
              <ButtonText
                ...
                invalid={inputInvalid}
                fontColor={editable?buttonFontColor:buttonDisableFontColor}
              >
                {submitButtonContent}
              </ButtonText>
            </Button>
